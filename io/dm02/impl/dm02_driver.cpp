...
extern "C" {
#include "io/dm02/protocol/channel/camera/camera_channel.h"
#include "io/dm02/protocol/channel/camera/camera_config.h"
#include "io/dm02/protocol/channel/gimbal/gimbal_channel.h"
#include "io/dm02/protocol/channel/gimbal/gimbal_config.h"
#include "io/dm02/protocol/channel/time_sync/time_sync_channel.h"
#include "io/dm02/protocol/channel/time_sync/time_sync_config.h"
#include "io/dm02/protocol/core/comm.h"
#include "io/dm02/protocol/core/comm_utils.h"
#include "io/dm02/protocol/core/uproto.h"
}
...
public:
  ...
  void on_mux(const std::uint8_t * pl, std::uint32_t len)
  {
    ...
    const std::int64_t offset =
      (static_cast<std::int64_t>(t1 - t0) + static_cast<std::int64_t>(t2 - t3)) / 2;

    if (max_rtt_us_ != 0 && rtt > max_rtt_us_) return;
...
struct Driver::Impl
{
  ...
  , config(std::move(cfg))
  , ts(TS_CH_ID, 1000u, true,
       [this](std::uint8_t ch, std::uint16_t sid, const std::uint8_t * p, std::uint16_t l) {
  ...
  std::optional<RefereeStatus> last_referee{};

  static void on_mux_trampoline(
    uproto_context_t *, std::uint16_t, const std::uint8_t * data, std::uint32_t len, void * user)
  {
  ...
  void init_uproto()
  {
    ...
    pop.get_mtu = [](void * u) -> std::uint16_t {
      const auto * self = static_cast<Impl *>(u);
  ...
  bool send_mux(std::uint8_t ch, std::uint16_t sid, const std::uint8_t * payload, std::uint16_t len)
  {
    ...
    hdr.sid = sid;
    hdr.seq = ++seqs[ch];
    const std::uint32_t wrote = mux_encode(buf, sizeof(buf), &hdr, payload);
    if (wrote == 0) return false;
    return uproto_send_notify(&uproto, UPROTO_MSG_MUX, buf, wrote) == UPROTO_OK;
  }
  ...
  void handle_mux(const std::uint8_t * data, std::uint32_t len)
  {
    ...
    if (sid == GIMBAL_SID_STATE) {
      ...
      const std::uint32_t base_words =
        (config.gimbal_has_encoders ? 2u : 0u) + (config.gimbal_has_imu ? 3u : 0u);
    ...
    if (sid == GIMBAL_SID_REFEREE) {
      if (hdr.len < 2 + 4 + 4 + 4 + 4 + 8) return;
      RefereeStatus rs{};
      rs.enemy_team = comm_read_i32_le(pl + 2);
      rs.fire_allowed = comm_read_i32_le(pl + 6);
      rs.robot_id = comm_read_i32_le(pl + 10);
      rs.game_stage = comm_read_i32_le(pl + 14);
      if (hdr.len >= 2 + 4 + 4 + 4 + 4 + 2 + 8) {
        rs.status = comm_read_u16_le(pl + 18);
        rs.device_ts_us = comm_read_u64_le(pl + 20);
      } else {
        rs.status = 0;
        rs.device_ts_us = comm_read_u64_le(pl + 18);
      }
      rs.host_ts_ns = now_ns_steady();
      last_referee = rs;
      if (callbacks.on_referee_status) callbacks.on_referee_status(rs);
      return;
    }
...
bool Driver::send_referee_query()
{
  std::uint8_t buf[2];
  comm_write_u16_le(&buf[0], GIMBAL_SID_REFEREE_QUERY);
  return send_mux(GIMBAL_CH_ID, GIMBAL_SID_REFEREE_QUERY, buf, sizeof(buf));
}

bool Driver::send_fire_command(const FireCommand & cmd)
{
  if (!impl_) return false;
  ...
  std::uint8_t buf[2 + 4 + 4 + 4 + 2 + 8];
  comm_write_u16_le(&buf[0], GIMBAL_SID_FIRE);
  comm_write_i32_le(&buf[2], cmd.fire_on);
  comm_write_i32_le(&buf[6], cmd.fire_mode);
  comm_write_i32_le(&buf[10], cmd.burst_count);
  comm_write_u16_le(&buf[14], cmd.status);
  comm_write_u64_le(&buf[16], ts_us);
  return send_mux(GIMBAL_CH_ID, GIMBAL_SID_FIRE, buf, sizeof(buf));
}

bool Driver::send_chassis_command(const ChassisCommand & cmd)
{
  if (!impl_) return false;
  ...
  std::uint8_t buf[2 + 4 + 4 + 4 + 4 + 2 + 8];
  comm_write_u16_le(&buf[0], GIMBAL_SID_CHASSIS);
  comm_write_i32_le(&buf[2], cmd.vx_mm_s);
  comm_write_i32_le(&buf[6], cmd.vy_mm_s);
  comm_write_i32_le(&buf[10], cmd.wz_mdeg_s);
  comm_write_i32_le(&buf[14], cmd.mode);
  comm_write_u16_le(&buf[18], cmd.status);
  comm_write_u64_le(&buf[20], ts_us);
  return send_mux(GIMBAL_CH_ID, GIMBAL_SID_CHASSIS, buf, sizeof(buf));
}
...
std::optional<RefereeStatus> Driver::last_referee_status() const
{
  if (!impl_) return std::nullopt;
  return impl_->last_referee;
}
