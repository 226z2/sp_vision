...
/**
   DM电机参数初始化
	 参数参考
   支持MIT协议才用 
   使用DM电机前必须初始化，否则电机会疯转
**/
__attribute__((used))void DM4310_param_init(MITMeasure_t *motor, float kp, float kd)
{
    if (motor == NULL) {
        return; // 防止空指针
    }
    ...
    // 1. 参数范围初始化（使用宏定义）
    motor->param.P_MIN = DM_P_MIN;      // -12.5 rad
    motor->param.P_MAX = DM_P_MAX;      // 12.5 rad
    motor->param.V_MIN = DM_V_MIN;      // -45 rad/s
    motor->param.V_MAX = DM_V_MAX;      // 45 rad/s
    motor->param.KP_MIN =DM_KP_MIN;    // 0.0
    motor->param.KP_MAX =DM_KP_MAX;    // 500.0
    motor->param.KD_MIN =DM_KD_MIN;    // 0.0
    motor->param.KD_MAX =DM_KD_MAX;    // 5.0
    motor->param.T_MIN = DM_T_MIN;      // -18 Nm
    motor->param.T_MAX = DM_T_MAX;      // 18 Nm
    
    // 2. 安全初始化控制命令（零状态）
    motor->set.POS = 0.0f;           // 零位置
    motor->set.VEL = 0.0f;           // 零速度
    motor->set.TOR = 0.0f;           // 零扭矩
    motor->set.KP = kp;           
    motor->set.KD = kd;           
    ...
    motor->fdb.Kp = 0.0f;
    motor->fdb.Kd = 0.0f;
    motor->fdb.t_mos = 0.0f;
    motor->fdb.t_motor = 0.0f;
    motor->fdb.last_fdb_time = 0;
	
}
	
	
MITMeasure_t *gimbal_DM2325_measure; //拨弹电机相关，暂时放这
...
__attribute__((used))void gimbal_init(gimbal_control_t *init)
{
    ...
    static const fp32 Yaw_speed_pid[3] = {YAW_SPEED_PID_KP, YAW_SPEED_PID_KI, YAW_SPEED_PID_KD};
		gimbal_PID_t gimbal_motor_DM_absolute_angle_pid;
    //电机数据指针获取
    init->gimbal_yaw_motor.gimbal_motor_measure = get_yaw_gimbal_motor_measure_point();
		init->gimbal_yaw_motor.gimbal_DM4310_measure = get_mit_motor_measure_point(0);  //大yaw指针
		DM4310_param_init(init->gimbal_yaw_motor.gimbal_DM4310_measure, 0, 1); //初始化KD为1
		
		
		gimbal_DM2325_measure = get_mit_motor_measure_point(1);  //获取拨弹电机指针
		DM4310_param_init(gimbal_DM2325_measure, 0, 2); //初始化KD为2
    ...
    init->gimbal_pitch_motor.gimbal_motor_mode = init->gimbal_pitch_motor.last_gimbal_motor_mode = GIMBAL_MOTOR_RAW;
		...
		Motor_clear_err(&hfdcan2, 0x01); //使能大yaw
		Motor_enable(&hfdcan2, 0x01);
		
		Motor_enable(&hfdcan1, 0x02);//使能拨弹
//		Motor_clear_err(&hfdcan1, 0x02);
		
		

		//DM电机的数据获取_大yaw
//		init->gimbal_yaw_motor.gimbal_motor_DM_measure = get_yaw_gimbal_motor_DM_measure_point(); 
		//DM大yaw外环PID
		gimbal_PID_init(&init->gimbal_yaw_motor.gimbal_DM4310_absolute_angle_pid, DM4310_YAW_GYRO_ABSOLUTE_PID_MAX_OUT, DM4310_YAW_GYRO_ABSOLUTE_PID_MAX_IOUT, DM4310_YAW_GYRO_ABSOLUTE_PID_KP, DM4310_YAW_GYRO_ABSOLUTE_PID_KI, DM4310_YAW_GYRO_ABSOLUTE_PID_KD);
    ...
    PID_init(&init->gimbal_yaw_motor.gimbal_motor_gyro_pid, PID_POSITION, Yaw_speed_pid, YAW_SPEED_PID_MAX_OUT, YAW_SPEED_PID_MAX_IOUT);
		PID_init(&init->gimbal_pitch_motor.gimbal_motor_gyro_pid, PID_POSITION, Pitch_speed_pid, PITCH_SPEED_PID_MAX_OUT, PITCH_SPEED_PID_MAX_IOUT);//内环用一套PID
    //清除所有PID
...
__attribute__((used))void gimbal_feedback_update(gimbal_control_t *feedback_update)
{
    ...
    feedback_update->gimbal_yaw_motor.motor_gyro = arm_cos_f32(feedback_update->gimbal_pitch_motor.relative_angle) * (*(feedback_update->gimbal_INT_gyro_point + INS_GYRO_Z_ADDRESS_OFFSET))
		...
		feedback_update->gimbal_yaw_motor.radian_of_ecd = rad_format(feedback_update->gimbal_yaw_motor.gimbal_DM4310_measure->fdb.pos);
...
__attribute__((used))void gimbal_set_control(gimbal_control_t *set_control)
{
    ...
    else if (set_control->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_GYRO)
    {
      //gyro模式下，陀螺仪角度控制
		  ...
		  rc_deadband_limit(set_control->gimbal_rc_ctrl->rc.ch[4], yaw_channel, RC_DEADBAND);
			
      set_control->gimbal_yaw_motor.max_relative_angle = 0.2f;  //手动设置小yaw的范围
		  set_control->gimbal_yaw_motor.min_relative_angle = -2.07f;
			
			gimbal_relative_angle_limit(&set_control->gimbal_yaw_motor, yaw_channel * YAW_RC_SEN);
    ...
    else if (set_control->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_ENCONDE) //刚上电时为这个模式
    {
        //enconde模式下，电机编码角度控制
    ...
    else if (set_control->gimbal_pitch_motor.gimbal_motor_mode == GIMBAL_MOTOR_ENCONDE)
    {
        ...
        set_control->gimbal_pitch_motor.max_relative_angle = 0.7f;
		    set_control->gimbal_pitch_motor.min_relative_angle = 0.2f;//
      #endif
        //enconde模式下，电机编码角度控制
...
/**DM4310电机
  ...
  * @retval         none
  */
void gimbal_DM4310_absolute_angle_control(gimbal_motor_t *gimbal_motor)
{
    fp32 out = 0;
    ...
    if (gimbal_motor == NULL)
    {
		...
		gimbal_motor->absolute_angle = rad_format(gimbal_motor->absolute_angle);
			...
			if(fabs(gimbal_motor->absolute_angle - gimbal_motor->absolute_angle_set) > PI)
			{
				...
				{
					out = gimbal_PID_Calc(&gimbal_motor->gimbal_DM4310_absolute_angle_pid, gimbal_motor->absolute_angle, gimbal_motor->absolute_angle_set + 2*PI, gimbal_motor->motor_gyro);
				}
				else
				{
					out = gimbal_PID_Calc(&gimbal_motor->gimbal_DM4310_absolute_angle_pid, gimbal_motor->absolute_angle, gimbal_motor->absolute_angle_set - 2*PI, gimbal_motor->motor_gyro);
			...
			{
				out = gimbal_PID_Calc(&gimbal_motor->gimbal_DM4310_absolute_angle_pid, gimbal_motor->absolute_angle, gimbal_motor->absolute_angle_set, gimbal_motor->motor_gyro);
			}
    ...
    last_out = out;
    //控制值赋值
    gimbal_motor->gimbal_DM4310_measure->set.VEL = (fp32)(out);
}

void gimbal_motor_auto_aim_relative_angle_control(gimbal_motor_t *gimbal_motor)
{
		fp32 out = 0;
    static float last_out = 0;
    if (gimbal_motor == NULL)
    {
      return;
		...
		gimbal_motor->relative_angle = rad_format(gimbal_motor->relative_angle);
		
		
			if(fabs(gimbal_motor->relative_angle - gimbal_motor->relative_angle_set) > PI)
			{
				if(gimbal_motor->relative_angle_set < 0)
				{
					out = -gimbal_PID_Calc(&gimbal_motor->gimbal_motor_relative_angle_pid, gimbal_motor->relative_angle, gimbal_motor->relative_angle_set + 2*PI, 0);
				}
				else
				{
					out = -gimbal_PID_Calc(&gimbal_motor->gimbal_motor_relative_angle_pid, gimbal_motor->relative_angle, gimbal_motor->relative_angle_set - 2*PI, 0);
			...
			{
				out = -gimbal_PID_Calc(&gimbal_motor->gimbal_motor_relative_angle_pid, gimbal_motor->relative_angle, gimbal_motor->relative_angle_set, 0);
			}
...
__attribute__((used))void gimbal_control_loop(gimbal_control_t *control_loop)
{
    ...
    else if (control_loop->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_GYRO)
    {
			  ...
			  gimbal_DM4310_absolute_angle_control(&control_loop->gimbal_yaw_motor);  //大yaw控制
		  	gimbal_motor_auto_aim_relative_angle_control(&control_loop->gimbal_yaw_motor);  //这里小yaw相对角控制

    }
    else if (control_loop->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_ENCONDE)
    {
        control_loop->gimbal_yaw_motor.given_current = 0; //哨兵不用这个
    }
    ...
    {
		  	gimbal_motor_auto_aim_relative_angle_control(&control_loop->gimbal_pitch_motor); 
...
__attribute__((used)) void gimbal_send_cmd(gimbal_control_t *control_send)
{
        ...
        pitch_can_set_current = -gimbal_control.gimbal_pitch_motor.given_current;
	      gimbal_DM2325_measure->set.VEL = shoot_can_set_current;
	  ...
	  control_send->gimbal_yaw_motor.gimbal_DM4310_measure->set.VEL = -control_send->gimbal_yaw_motor.gimbal_DM4310_measure->set.VEL;
		CAN_cmd_MIT(&hfdcan2, 0x01, *control_send->gimbal_yaw_motor.gimbal_DM4310_measure);
		CAN_cmd_MIT(&hfdcan1, 0x02, *gimbal_DM2325_measure);
    CAN_cmd_gimbal(yaw_can_set_current, pitch_can_set_current, 0, 0);
				
//		if(INS.ins_flag == 0){ yaw_can_set_current = 0; pitch_can_set_current = 0; }
//		CAN_cmd_gimbal(yaw_can_set_current, pitch_can_set_current, 0, 0);
}
