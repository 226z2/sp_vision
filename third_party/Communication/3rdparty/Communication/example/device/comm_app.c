...
static fp32 imu_angle_pick(const fp32 *angles, int idx) {
    if(!angles)
        return 0.0f;
    if(idx < 0 || idx > 2)
        return 0.0f;
...
static void setup_channels(void) {
    ...
    camera_channel_init_ex(&g_camera, &g_bind, &g_mgr, &cam_cfg, (uint64_t (*)(void *))tim24_timebase_now_us, NULL, NULL);
    ...
    gimbal_channel_init_ex(&g_gimbal, &g_bind, &g_mgr, &gcfg, &gsrc, &ghk, (uint64_t (*)(void *))tim24_timebase_now_us, NULL);
 ...
 */
static void tfmini_tick(void) {
    uint64_t now = tim24_timebase_now_us();
...
static bool gimbal_get_state(gimbal_state_t *out, void *user) {
    ...
    fp32 *angles = get_INS_angle_point();
    ...
    if(angles) {
        const float yaw = imu_angle_pick(angles, COMM_IMU_YAW_IDX) * (float)COMM_IMU_YAW_SIGN;
        const float pitch = imu_angle_pick(angles, COMM_IMU_PITCH_IDX) * (float)COMM_IMU_PITCH_SIGN;
        const float roll = imu_angle_pick(angles, COMM_IMU_ROLL_IDX) * (float)COMM_IMU_ROLL_SIGN;
        out->yaw_udeg = (int32_t)(yaw * 1800000.0f * INV_PI);
        out->pitch_udeg = (int32_t)(pitch * 1800000.0f * INV_PI);
        out->roll_udeg = (int32_t)(roll * 1800000.0f * INV_PI);
    } else {
        out->yaw_udeg = out->pitch_udeg = out->roll_udeg = 0;
    }
    out->ts_us = tim24_timebase_now_us();
    return true;
}
