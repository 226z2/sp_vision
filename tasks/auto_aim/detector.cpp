...
std::list<Armor> Detector::detect(const cv::Mat & bgr_img, int frame_count)
{
  ...
  publish_debug_image(binary_img, "detector/binary");
...
void Detector::save(const Armor & armor) const
{
  ...
  auto img_path = fmt::format("{}/{}_{}.jpg", save_path_, armor_name_to_string(armor.name), file_name);
  ...
  const std::list<Armor> & armors, int frame_count) const
{
  ...
  publish_debug_image(binary_img2, "detector/threshold");
  publish_debug_image(detection, "detector/detection");
}
